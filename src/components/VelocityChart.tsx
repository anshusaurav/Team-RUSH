...
export default function VelocityChart({ data }: Props) {
  ...
  const chartData = [
    ...
    { label: 'Next Wk', actual: null as number | null, predicted },
  ];
  ...
  const stockoutLabel =
    days_to_stockout !== null
      ? days_to_stockout < 7
        ? `⚠ ${days_to_stockout}d to stockout`
        : days_to_stockout < 14
        ? `${days_to_stockout}d stock left`
        : null
      : null;
    ...
    <div className="space-y-2">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400">Weekly sales velocity · LSTM forecast</p>
        </div>
          ...
          {anomaly_type && (
            <div>
              <span className={`inline-block text-xs font-semibold px-2 py-0.5 rounded-full ${
                ...
                {isSpike ? `🧠 ${deviation}× spike` : '🧠 Velocity drop'}
              </span>
            </div>
          )}
        </div>
      ...
      <div className="flex items-center justify-between text-xs text-gray-500 pt-1 border-t border-gray-100">
        <span>Stock: <span className="font-semibold text-gray-700">{current_qty} units</span></span>
        <span>Forecast next wk: <span className="font-semibold" style={{ color: predColor }}>{predicted} units</span></span>
      </div>
    </div>
  );
}
